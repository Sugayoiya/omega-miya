...
class TraitorRender(StickerRender):
    ...
    _static_resource = STATIC_RESOURCE('traitor', 'default_bg.png')
    _font = FONT_RESOURCE('pixel.ttf')
    ...
    def _handler(self) -> bytes:
        ...
        image = self._resize_to_width(image=image, width=self._default_output_width)
        text = self.text[:100]
...
class JichouRender(StickerRender):
    ...
    _static_resource = STATIC_RESOURCE('jichou', 'default_bg.png')
    _font = FONT_RESOURCE('SourceHanSansSC-Regular.otf')
...
class PhlogoRender(StickerRender):
    ...
    _font = FONT_RESOURCE('SourceHanSansSC-Heavy.otf')
...
class LuxunSayRender(StickerRender):
    ...
    _static_resource = STATIC_RESOURCE('luxunsay', 'default_bg.png')
    _font = FONT_RESOURCE('SourceHanSansSC-Regular.otf')
    ...
    def _handler(self) -> bytes:
        ...
        text_ = ImageUtils.split_multiline_text(
            text=self.text, width=text_width_limit, font=font, stroke_width=text_stroke_width
        )
...
class LuxunWriteRender(LuxunSayRender):
    ...
    _static_resource = STATIC_RESOURCE('luxunwrite', 'default_bg.png')
...
class JiangzhuangRender(StickerRender):
    ...
    _static_resource = STATIC_RESOURCE('jiangzhuang', 'default_bg.png')
    _font = FONT_RESOURCE('HanYiWeiBeiJian.ttf')
    ...
    def _handler(self) -> bytes:
        ...
        # 分割文本
        text_ = ImageUtils.split_multiline_text(text=self.text, width=text_width_limit, font=font)
...
class XibaoHorizontalRender(StickerRender):
    ...
    _static_resource = STATIC_RESOURCE('xibaoh', 'default_bg.png')
    _font = FONT_RESOURCE('HanYiWeiBeiJian.ttf')
    _default_output_width = 1024
...
class XibaoVerticalRender(StickerRender):
    ...
    _static_resource = STATIC_RESOURCE('xibaos', 'default_bg.png')
    _font = FONT_RESOURCE('SourceHanSerif-Bold.ttc')
    _default_output_width = 1024
...
class DefaultRender(StickerRender):
    ...
    _font = FONT_RESOURCE('msyhbd.ttc')
    ...
    def _static_handler(self, image: Image) -> bytes:
        ...
        text_w, text_h = ImageUtils.get_text_size(self.text, font=font, stroke_width=text_stroke_width)
        # 自适应处理文字大小
...
class WhiteBackgroundRender(StickerRender):
    ...
    def _static_handler(self, image: Image) -> bytes:
        ...
        font = ImageFont.truetype(self._font.resolve_path, font_size)
        text_w, text_h = ImageUtils.get_text_size(self.text, font=font)
        # 自适应处理文字大小
...
class GunjoRender(StickerRender):
    ...
    _font = FONT_RESOURCE('SourceHanSansSC-Bold.otf')
...
class MarriageRender(StickerRender):
    ...
    _static_resource = STATIC_RESOURCE('marriage', 'default_bg.png')
    _need_text: bool = False
    _need_external_img: bool = True
    ...
    def _static_handler(self, image: Image, *, resize_width: int | None = None) -> bytes:
        ...
        if resize_width:
            background = self._resize_to_width(image=background, width=resize_width)
        content = self._get_pil_image(image=background)
        return content
        ...
        if fm == 'GIF':
            ...
            while True:
                try:
                    ...
                    image = ImageUtils(image=image).resize_with_filling(
...
class GrassJaRender(StickerRender):
    ...
    _font = FONT_RESOURCE('fzzxhk.ttf')
    _need_external_img: bool = True
    ...
    async def _translate_preprocessor(self) -> None:
        ...
        text_trans_result = await TencentTMT().text_translate(source_text=self.text, target='ja')
...
class PetPetRender(StickerRender):
    ...
    _static_resource = STATIC_RESOURCE('petpet')
    _need_text: bool = False
    _need_external_img: bool = True
...
class WorshipRender(StickerRender):
    ...
    _static_resource = STATIC_RESOURCE('worship')
    _need_text: bool = False
    _need_external_img: bool = True
    _default_output_format: str = 'gif'
    ...
    def _handler(self) -> bytes:
        ...
        p_image = image.transform(
            size=image.size, method=Image.Transform.PERSPECTIVE, data=perspective_data, fill=Image.Resampling.BICUBIC
        )
...
class TwistRender(StickerRender):
    ...
    """
    _sticker_name: str = 'worship'
    _static_resource = STATIC_RESOURCE('twist')
    _need_text: bool = False
    _need_external_img: bool = True
    _default_output_format: str = 'gif'
    ...
    def _handler(self) -> bytes:
        image = self._load_source_image()
        image = ImageUtils(image=image).resize_with_filling(size=(128, 128)).image
...
class WangjingzeRender(StickerRender):
    ...
    _font = FONT_RESOURCE('SourceHanSansSC-Regular.otf')
    _static_resource = STATIC_RESOURCE('wangjingze')
